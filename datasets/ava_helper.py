...
def load_image_lists(cfg, is_train):
    ...
    """
    # ["/home/lixin/FC3D/data/ava/frame_lists/train.csv"]
    # 获取视频转化的所有图片的路径，为一个列表
    list_filenames = [
    ...
    for list_filename in list_filenames:
        with pathmgr.open(list_filename, "r") as f:
            ...
            for line in f:
                ...
                # {0:[pic1，pic2，pic3, ...],1:[],2:[],...} 一个视频的所有图片路径在一个key中
                image_paths[data_key].append(
                    os.path.join(cfg.AVA.FRAME_DIR, row[3])
                )

    image_paths = [image_paths[i] for i in range(len(image_paths))] # 将字典转化为列表
...
def load_boxes_and_labels(cfg, mode):
    ...
    Returns:
        all_boxes (dict): a dict which maps from `video_name` and
            ...
            labels for the box.
    """
    gt_lists = cfg.AVA.TRAIN_GT_BOX_LISTS if mode == "train" else cfg.AVA.VAL_GT_BOX_LISTS if mode == "val" else []
    pred_lists = (
        cfg.AVA.TRAIN_PREDICT_BOX_LISTS
        if mode == "train"
        else cfg.AVA.TEST_PREDICT_BOX_LISTS
    )
    # /home/lixin/FC3D/data/ava/annotations/ava_train_v2.2.csv
    ann_filenames = [
    ...
    detect_thresh = cfg.AVA.DETECTION_SCORE_THRESH # 不用
    ...
    for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box):
        with pathmgr.open(filename, "r") as f:
            for line in f:
                ...
                box_key = ",".join(row[2:6]) # 坐标
                box = list(map(float, row[2:6])) # [x1,y1,x2,y2]

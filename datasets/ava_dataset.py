...
class Ava(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        """
        # Loading frame paths.
        # [[pic_path1,pic_path2,pic_path3,pic_path4,...],[pic_path1,pic_path2,pic_path3,pic_path4,...],[]pic_path1,pic_path2,pic_path3,pic_path4,...,[pic_path1,pic_path2,pic_path3,pic_path4,...]]
        (
            self._image_paths,
    ...
    def __getitem__(self, idx):
        ...
        path = self._image_paths[video_idx][center_idx]
        ...
        if self._split == "train":
            for box_labels in clip_label_list:
                b_labels = [bl-1 for bl in box_labels[1]]
                label_map[idx + int(sum(box_labels[0]) * 1000)] = b_labels
                targets.append([idx + int(sum(box_labels[0]) * 1000)] + box_labels[0])
        else:
            for box_labels in clip_label_list:
                for lb in box_labels[1]:
                    targets.append([lb-1] + box_labels[0])
        ...
        #         if label == -1:
        #             continue
        #         assert label >= 1 and label <=80 # 这个地方根据实际情况进行更改
